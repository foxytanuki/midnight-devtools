...
export function WalletIndexedData() {
	...
	const handleConnect = useCallback(async () => {
		if (!connectedAPI || !graphqlClientRef.current) {
			setError("Wallet is not connected or GraphQL client is not initialized");
			return;
		}
		...
		try {
			...
			if (graphqlClientRef.current) {
				shieldedSubscriptionRef.current = graphqlClientRef.current.subscribe({
					...
					onNext: (data: unknown) => {
						const event = (data as { shieldedTransactions: ShieldedTransactionsEvent })
							.shieldedTransactions;

						if (event.__typename === "RelevantTransaction") {
							setShieldedTransactions((prev) => [...prev, event]);
						} else if (event.__typename === "ShieldedTransactionsProgress") {
							setShieldedProgress(event);
						}
					},
					onError: (err) => {
			...
			if (err instanceof ViewingKeyError) {
				...
				console.log("Viewing key not available, skipping shielded transactions");
			} else {
				setError(
	...
	const handleDisconnect = useCallback(async () => {
		...
		try {
			...
			setError(
				err instanceof Error ? err.message : "Failed to disconnect from indexer",
			);
		}
	}, [sessionId]);
		...
		unshieldedSubscriptionRef.current = graphqlClientRef.current.subscribe({
			...
			onNext: (data: unknown) => {
				...
				if (event.__typename === "UnshieldedTransaction") {
					setUnshieldedTransactions((prev) => [...prev, event]);
				} else if (event.__typename === "UnshieldedTransactionsProgress") {
					setUnshieldedProgress(event);
				}
			},
			onError: (err) => {
				...
				<>
					...
					{unshieldedAddress && (
						...
						>
							...
							<p className="method-description-text">
								Unshielded transactions are available without viewing key. They are
								indexed by address and updated in real-time. NIGHT (the native token,
								tNIGHT on preview/testnet networks) is used in unshielded transactions.
							</p>
							{unshieldedProgress && (
								<div className="info-box" style={{ marginBottom: "1rem" }}>
									<strong>Indexing Progress:</strong>
									<br />
									Highest Transaction ID: {unshieldedProgress.highestTransactionId}
								</div>
							)}
							{unshieldedTransactions.length === 0 ? (
								...
								>
									{[...unshieldedTransactions]
										.sort((a, b) => {
											...
											return b.transaction.block.height - a.transaction.block.height;
										})
										.map((item, index) => {
										...
										const getTransactionType = (): {
											...
											if (item.spentUtxos.length === 0 && item.createdUtxos.length > 0) {
												return {
													type: "Receive",
											...
											const spentOwners = new Set(
												item.spentUtxos.map((utxo) => utxo.owner?.toLowerCase() || ""),
											);
											const createdOwners = new Set(
												item.createdUtxos.map((utxo) => utxo.owner?.toLowerCase() || ""),
											);
											...
											if (
												...
												return {
													...
													description: "UTXO consolidation (same address, zero net change)",
												};
											}
										...
										return (
											<div key={index} className="unshielded-transaction-card">
												<div className="result-item-header">
													<div style={{ display: "flex", alignItems: "center", gap: "0.75rem", flexWrap: "wrap" }}>
														...
														{item.transaction.block.timestamp && (
															...
															>
																{formatTimestamp(item.transaction.block.timestamp)}
															</span>
														)}
													</div>
													<div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
														{item.transaction.block.height && (
															<a
																href={getBlockExplorerUrl(item.transaction.block.height)}
																target="_blank"
												...
												<div className="utxo-summary">
																		...
																		{utxo.owner && (
																			<div className="utxo-owner">
																				...
																				<button
																					type="button"
																					onClick={() => {
																						navigator.clipboard.writeText(utxo.owner);
																					}}
																					className="copy-button"
													...
													{(item.createdUtxos.length > 0 || item.spentUtxos.length > 0) && (
														<div className="utxo-net">
															<span className="utxo-net-label">Net Change:</span>
															<span
