...
// NetworkId型を定義（パッケージから直接インポートできない場合の代替）
...
export function decodeBech32mAddress(address: string): ShieldedAddress | null {
	try {
		...
		if (
			!address.startsWith("mn_shield-addr_") &&
			!address.startsWith("addr_")
		) {
			console.warn(
				"Address does not start with expected prefix:",
		...
		const networkId: NetworkId = address.includes("test")
			? "testnet-02"
			: "mainnet";
