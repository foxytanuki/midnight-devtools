...
export async function getViewingKey(
	...
	// #region agent log
	fetch("http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da", {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({
			location: "wallet-indexer.ts:29",
			message: "getViewingKey called",
			data: {
				hasConnectedAPI: !!connectedAPI,
				connectedAPIType: typeof connectedAPI,
				connectedAPIKeys: connectedAPI ? Object.keys(connectedAPI) : [],
			},
			timestamp: Date.now(),
	...
	fetch("http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da", {
		...
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({
			location: "wallet-indexer.ts:37",
			message: "Checking API methods",
			data: {
				apiKeys,
	...
	if (
		"getViewingKey" in connectedAPI &&
		typeof connectedAPI.getViewingKey === "function"
	) {
		try {
			// #region agent log
			fetch(
				"http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da",
				{
					method: "POST",
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						location: "wallet-indexer.ts:42",
						message: "Calling getViewingKey method",
						data: {},
						timestamp: Date.now(),
						sessionId: "debug-session",
						runId: "run1",
			...
			fetch(
				"http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da",
				{
					...
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						location: "wallet-indexer.ts:46",
						message: "getViewingKey succeeded",
						data: {
							viewingKeyLength: viewingKey?.length,
							viewingKeyPrefix: viewingKey?.substring(0, 20),
						},
						timestamp: Date.now(),
						sessionId: "debug-session",
						runId: "run1",
			...
			fetch(
				"http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da",
				{
					...
					headers: { "Content-Type": "application/json" },
					body: JSON.stringify({
						location: "wallet-indexer.ts:50",
						message: "getViewingKey failed",
						data: {
							errorMessage:
	...
	fetch("http://localhost:7243/ingest/97c327b1-c108-4daa-8275-24875ac111da", {
		...
		headers: { "Content-Type": "application/json" },
		body: JSON.stringify({
			location: "wallet-indexer.ts:58",
			message: "getViewingKey method not found",
